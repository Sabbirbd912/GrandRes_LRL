...
// }
	...
	// 		->join('transaction_types', 'transaction_types.id', '=', 'stocks.transaction_type_id')
	// 		->join('raw_materials', 'raw_materials.id', '=', 'stocks.raw_material_id')
	// 		->select('raw_materials.id', 'raw_materials.name as raw_material', DB::raw('SUM(stocks.qty) as total'))
	// 		->groupBy('stocks.raw_material_id', 'raw_materials.id', 'raw_materials.name')
	// 		->get();
    ...
    public function balance()
    {
        $result = DB::table('stocks')
            ...
            ->select(
                'raw_materials.id',
                'raw_materials.name as raw_material',
                // এখানে পরিবর্তন: stocks.qty এর পরিবর্তে core_stocks.qty
                DB::raw('SUM(CASE WHEN core_transaction_types.name = "In" THEN core_stocks.qty ELSE -core_stocks.qty END) as total')
            )
            ->groupBy('stocks.raw_material_id', 'raw_materials.id', 'raw_materials.name')
            ->get();
